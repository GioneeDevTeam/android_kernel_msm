...
#if defined(CONFIG_GN_Q_BSP_LCD_CABC_CHANGE_SUPPORT)
#include <linux/leds.h>
#define LCD_CABC_MAX_NUM 1

extern u32 panel_vendor;
...
void set_backlight_lm3630(unsigned int level)
{
	...
	if(panel_vendor == 2)
		para = (u8) ((level*17)/20);
	else if(panel_vendor == 1)  //for tianma lcd
		para = (u8) ((level*9)/10);
	else
		para = (u8) ((level*3)/4);
