...
hw_shutdown:
	if (rmi4_data->board->regulator_en) {
		if (regulator_count_voltages(rmi4_data->vdd) > 0)
			regulator_set_voltage(rmi4_data->vdd, 0, 0);
		regulator_put(rmi4_data->vdd);
	}
	if (rmi4_data->board->i2c_pull_up) {
		if (regulator_count_voltages(rmi4_data->vcc_i2c) > 0)
			regulator_set_voltage(rmi4_data->vcc_i2c, 0, 0);
		regulator_put(rmi4_data->vcc_i2c);
	}
	return 0;
};
...
static int synaptics_rmi4_suspend(struct device *dev)
{
    ...
    if (5 == power_on_flag) {
        ...
        retval = synaptics_rmi4_regulator_configure(rmi4_data, false);
	    if (retval < 0) {
		    dev_err(dev, "tpd: failed to shutdown power\n");
		    return retval;
	    }
        power_on_flag = 0;
    }
...
//Release Finger
    ...
    printk("tpd: synaptics_rmi4_resume... Success!\n");
    suspend_flag = 0;
#ifdef TP_GLOVE_SUPPORT
    if (glove_enable) {
        do {
